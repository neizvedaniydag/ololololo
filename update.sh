...
# Поиск всех папок содержащих education_platform в названии и имеющих app.py
while IFS= read -r -d '' dir; do
    if [ -f "$dir/app.py" ]; then
        FLASK_FOLDERS+=("$dir")
    fi
done < <(find "$TEMP_REPO" -maxdepth 2 -type d -iname "*education_platform*" -print0 2>/dev/null)

# Если не найдено папок с education_platform, проверяем корень
if [ ${#FLASK_FOLDERS[@]} -eq 0 ] && [ -f "$TEMP_REPO/app.py" ]; then
    FLASK_FOLDERS+=("$TEMP_REPO")
fi

# Проверяем что нашли хоть что-то
if [ ${#FLASK_FOLDERS[@]} -eq 0 ]; then
    error_exit "Не найдено папок с app.py в репозитории!"
fi

echoc "   → Найдено папок: ${#FLASK_FOLDERS[@]}" $C_YELLOW
...
if [ "$AUTO_SELECT" = true ]; then
    ...
    echoc "   ✓ Автовыбрана последняя папка: $(basename "$FLASK_APP_DIR")" $C_GREEN
else
    # Показываем список для выбора
    if [ ${#SORTED_FOLDERS[@]} -eq 1 ]; then
        # Если папка всего одна, выбираем автоматически
        FLASK_APP_DIR="${SORTED_FOLDERS[0]}"
        echoc "   ✓ Найдена единственная папка: $(basename "$FLASK_APP_DIR")" $C_GREEN
    else
        # Несколько папок - даём выбрать
        echo ""
        echoc "   Найдено несколько папок с Flask-приложением:" $C_CYAN
        echo ""
        ...
        echoc "   ✓ Выбрана папка: $(basename "$FLASK_APP_DIR")" $C_GREEN
    fi
fi
...
# Копируем весь контент Flask-приложения
echoc "   → Копирование файлов из $(basename "$FLASK_APP_DIR")..." $C_YELLOW
...
echoc "🌿 Бранч: $BRANCH" $C_YELLOW
echoc "📁 Папка: $(basename "$FLASK_APP_DIR")" $C_CYAN
