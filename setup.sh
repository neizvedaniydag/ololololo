...
SSL_SUCCESS=false
SSL_OUTPUT=$($DC run --rm -p 80:80 --entrypoint "\
...
if echo "$SSL_OUTPUT" | grep -q "too many certificates"; then
    ...
    echoc "   → Оставляем HTTP версию" $C_YELLOW
    SSL_SUCCESS=false
...
EOF
        ...
        echoc "   ⚠ Сертификат не найден, оставляем HTTP" $C_RED
        SSL_SUCCESS=false
    fi
else
...
if $DC ps | grep nginx | grep -q "Up"; then
    echoc "   ✓ Nginx запущен" $C_GREEN
else
    echoc "   ⚠ Nginx упал, откатываюсь к HTTP..." $C_RED
    # Вернуть HTTP конфиг
    cat > nginx/production.conf <<EOF
server {
    listen 80;
