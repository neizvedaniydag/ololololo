export const pushup = {
    ...
    thresholds: {
        // ОЧЕНЬ МЯГКИЕ пороги - засчитает ЛЮБОЕ движение
        elbowDown: 140,     // Если угол меньше 140° - считается опускание
        elbowUp: 150        // Если угол больше 150° - считается подъём
    },
    ...
    analyze(lm, state, showHint, logError, calcAngle) {
        ...
        if (elbow < this.thresholds.elbowDown && state.position === 'up') {
            ...
            state.position = 'down';
            result.counted = true;
            result.correct = true;  // ВСЕГДА ПРАВИЛЬНО!
            ...
            state.position = 'up';
            result.status = 'Готов!';
            showHint('Готов к следующему', this.svgIcons.bodyDown, 'rgba(16, 185, 129, 0.95)');
        }
        else {
            // ПРОМЕЖУТОЧНОЕ
            if (state.position === 'up') {
                ...
                result.status = `Поднимайтесь (${Math.round(elbow)}°)`;
            }
        }
