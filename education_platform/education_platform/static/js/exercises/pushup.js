...
analyze(lm, state, showHint, logError, calcAngle) {
    ...
    // ========== КРИТИЧЕСКАЯ БЛОКИРОВКА: НЕТ КАЛИБРОВКИ = НЕТ ПОДСЧЕТА ==========
    if (state.calibrationStep < 3) {
        // ЕСЛИ КАЛИБРОВКА НЕ ЗАВЕРШЕНА - ПОДСЧЕТ ЗАПРЕЩЕН!
        // Переходим сразу к логике калибровки
    } else {
        // КАЛИБРОВКА ЗАВЕРШЕНА - можно считать
    }
    ...
    // ========== РЕЖИМ КАЛИБРОВКИ (ШАГ 0, 1, 2) ==========
    if (state.calibrationStep < 3) {
        
        // ВСЯ ЛОГИКА КАЛИБРОВКИ ЗДЕСЬ
        
        if (bodyAspectRatio < this.thresholds.minBodyAspectRatio) {
            result.status = `🚫 ЛЯГТЕ В ПЛАНКУ! Соотношение: ${bodyAspectRatio.toFixed(1)}`;
        ...
        if (!isHorizontal || kneeAngle < this.thresholds.kneeAngleMin) {
            result.status = `🚫 ПРИМИТЕ ПОЛОЖЕНИЕ ПЛАНКИ!`;
        ...
        if (state.calibrationStep === 0) {
            ...
            result.status = `Локоть: ${elbow}° Тело: ${bodyAngle}°`;
            showHint(
            ...
            return result; // ВАЖНО: выходим до подсчета!
        }
    ...
    // ========== РЕЖИМ ПОДСЧЕТА (ТОЛЬКО ПОСЛЕ КАЛИБРОВКИ!) ==========
    // Сюда попадаем ТОЛЬКО если calibrationStep === 3
    ...
    if (noseToAnkleDist < this.thresholds.minNoseToAnkleDistance) {
        state.consecutiveValidFrames = 0;
        state.failedChecks++;
        result.status = `❌❌ ТЕЛО НЕ РАСТЯНУТО!`;
    ...
    if (shoulderY < this.thresholds.shoulderMaxYPosition) {
        state.consecutiveValidFrames = 0;
        state.failedChecks++;
        result.status = `❌❌ ПЛЕЧИ СЛИШКОМ ВЫСОКО!`;
